...
const MyItems = () => {
    ...
    return (
        <div className='grid md:grid-cols-2 grid-cols-1 lg:grid-cols-3 2xl:grid-cols-3 mt-8'>
            ...
            {
                books.length ?
                    books.map(book => <Book book={book} key={book._id}><button onClick={() => deleteBook(book._id)} className='py-3 font-semibold px-8 rounded-3xl  bottom-0 bg-red-500 mt-4 text-white'>Delete</button></Book>)
                    :
